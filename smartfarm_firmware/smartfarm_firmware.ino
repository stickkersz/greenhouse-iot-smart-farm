/*
  ...
  Hardware:
    - ESP32 DevKit V1
    - SHT35 (I2C: SDA=21, SCL=22) — อุณหภูมิ + ความชื้นอากาศ
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n=== Greenhouse IoT Smart Farm v1.2.0 ===");
  ...
  if (sht35.begin(0x44)) {
    ...
    dht11.begin();
    Serial.println("SHT35 ไม่พบ — ใช้ DHT11 แทน");
  }
...
void readSensors() {
  if (useSHT35) {
    ...
    if (isnan(airTemp) || isnan(airHumidity)) {
      Serial.println("[SHT35] NaN — ตรวจ Wire.begin() และ wiring");
    ...
    if (isnan(airTemp) || isnan(airHumidity)) {
      Serial.println("[DHT11] NaN error");
      airTemp = 0; airHumidity = 0;
    }
  }
...
void autoControl() {
  ...
  bool shouldOpen  = (airTemp >= ton)  || (airHumidity < hmin);
  bool shouldClose = (airTemp <= toff) && (airHumidity >= hmin);

  // CH1 Pump
  if (ch_isAuto[0]) {
    if (shouldOpen  && !ch1_pump) { ch1_pump = true;  setRelay(PIN_RELAY_CH1, true);  }
    if (shouldClose &&  ch1_pump) { ch1_pump = false; setRelay(PIN_RELAY_CH1, false); }
  }
  // CH2 Fan Out
  if (ch_isAuto[1]) {
    if (shouldOpen  && !ch2_fanOut) { ch2_fanOut = true;  setRelay(PIN_RELAY_CH2, true);  }
    if (shouldClose &&  ch2_fanOut) { ch2_fanOut = false; setRelay(PIN_RELAY_CH2, false); }
  }
  // CH3 Fan In
  if (ch_isAuto[2]) {
    if (shouldOpen  && !ch3_fanIn) { ch3_fanIn = true;  setRelay(PIN_RELAY_CH3, true);  }
    if (shouldClose &&  ch3_fanIn) { ch3_fanIn = false; setRelay(PIN_RELAY_CH3, false); }
  }
  // CH4 Spare (รองรับ auto แต่ default = manual)
  if (ch_isAuto[3]) {
    if (shouldOpen  && !ch4_spare) { ch4_spare = true;  setRelay(PIN_RELAY_CH4, true);  }
    if (shouldClose &&  ch4_spare) { ch4_spare = false; setRelay(PIN_RELAY_CH4, false); }
  }

  if (shouldOpen)  Serial.printf("[AUTO] เปิด — T:%.1f≥%.1f หรือ RH:%.1f<%.1f\n", airTemp, ton, airHumidity, hmin);
  if (shouldClose) Serial.printf("[AUTO] ปิด  — T:%.1f≤%.1f และ RH:%.1f≥%.1f\n", airTemp, toff, airHumidity, hmin);
}

// ─────────────────────────────────────────────────────
// Apply Manual Control — เรียกหลัง loadControlFromFirebase()
void applyManualControl() {
  if (!ch_isAuto[0] && (bool)ch_manual[0] != ch1_pump) {
    ch1_pump = ch_manual[0];
    setRelay(PIN_RELAY_CH1, ch1_pump);
    Serial.printf("[MANUAL] CH1 Pump → %s\n", ch1_pump ? "ON" : "OFF");
  }
  if (!ch_isAuto[1] && (bool)ch_manual[1] != ch2_fanOut) {
    ch2_fanOut = ch_manual[1];
    setRelay(PIN_RELAY_CH2, ch2_fanOut);
    Serial.printf("[MANUAL] CH2 Fan OUT → %s\n", ch2_fanOut ? "ON" : "OFF");
  }
  if (!ch_isAuto[2] && (bool)ch_manual[2] != ch3_fanIn) {
    ch3_fanIn = ch_manual[2];
    setRelay(PIN_RELAY_CH3, ch3_fanIn);
    Serial.printf("[MANUAL] CH3 Fan IN → %s\n", ch3_fanIn ? "ON" : "OFF");
  }
  if (!ch_isAuto[3] && (bool)ch_manual[3] != ch4_spare) {
...
void pushToFirebase() {
  ...
  Firebase.setString(fbData, base + "status/firmware",    "1.2.0");
...
void checkAlerts() {
  if (airTemp > 38.0) {
    ...
    Firebase.setString(fbData, "/smartfarm/alerts/last_alert/message",
      "อุณหภูมิสูงเกิน 38°C! (" + String(airTemp, 1) + "°C)");
    Serial.println("[ALERT] High Temp: " + String(airTemp, 1) + "°C");
  }
  if (airHumidity > 0 && airHumidity < 40.0) {
    ...
    Firebase.setString(fbData, "/smartfarm/alerts/last_alert/message",
      "ความชื้นต่ำมาก! (" + String(airHumidity, 1) + "%)");
